...
GLuint IBO; //использую указатель для доступа к всемирной матрице, представленной в виде uniform - переменной внутри шейдера.
GLuint gWVPLocation;
...
static void RenderSceneCB()
{
    ...
    Pipeline p;
    p.Rotate(0.0f, Scale*90, 0.0f); //кручение
    p.WorldPos(sinf(Scale), 0.0f, 0.0f); //передвижение по оси X
    p.Scale(2.0f, 2.0f, 2.0f); //увеличен масштаб
    Vector3f CameraPos(0.0f, 2.0f, -3.0f);
    Vector3f CameraTarget(0.0f, -1.0f, 2.0f);
    Vector3f CameraUp(0.0f, 1.0f, 0.0f);
    p.SetCamera(CameraPos, CameraTarget, CameraUp);
    p.SetPerspectiveProj(60.0f, WINDOW_WIDTH, WINDOW_HEIGHT, 1.0f, 100.0f); //параметры проекции
...
{
    GLuint ShaderObj = glCreateShader(ShaderType); //создание шейдеров
    ...
    GLint success; //компиляция шейдера
    ...
    glAttachShader(ShaderProgram, ShaderObj); //присоединяю скомпилированный объект шейдера к объекту программы
}
...
static void CompileShaders()
{
    ...
    glLinkProgram(ShaderProgram); //линкую
    ...
    glValidateProgram(ShaderProgram); //еще проверяю
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Tutorial 12"); //заголовок
